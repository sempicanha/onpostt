...
var onpostt = {
	...
	sendBlock: async function (block, callbacks) {
	    if (!this.isValidJsonObject(block)) {
	        console.log("❌ Bloco inválido: não pode ser convertido para JSON.");
	        return;
	    }
	    ...
	    for (let socket of this.sockets) {
	        if (socket.readyState === WebSocket.OPEN) {
	            ...
	            socket.onmessage = function (event) {
	                // console.log('📩 Mensagem recebida do Relay:', event.data);

	                try {
	                    const response = JSON.parse(event.data);
	                    if (response.status === 'erro') {

	                        console.log('❌ Erro do Relay:', response.message);
