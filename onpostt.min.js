...
var onpostt = {
    ...
    // Gerar chave a partir de uma entrada (exemplo de uma string customizada)
    generateKeysCustom: function (input) {
        const hash = this.sha256(input); // Função SHA-256 personalizada
        const keyPair = this.ec.keyFromPrivate(hash);
        return {
            privateKey: keyPair.getPrivate('hex'),
            publicKey: keyPair.getPublic(true, 'hex') // Compressão ativada
        };
    ...
    sendBlock: async function (block, callbacks) {
        // console.log('sendBlock:', block);

        // Verifica se o bloco é um objeto JSON válido
        if (!this.isValidJsonObject(block)) {
            console.error("❌ Bloco inválido: não pode ser convertido para JSON.");
            return;
        }
        ...
        // Envia o bloco para todos os Relay conectados
        let socketsWithError = [];
        for (let socket of this.sockets) {
            if (socket.readyState === WebSocket.OPEN) {
                let formatBlock = { ...block };

                // Salva a assinatura antes de removê-la para validação
                ...
                if (!status) {
                    console.log('❌ Assinatura inválida, bloco rejeitado!');
                    continue;
                }

                // Envia o bloco para o Relay
                socket.send(JSON.stringify(block));
                console.log('✅ Bloco enviado para o Relay');

                // Recebe as mensagens do Relay
                socket.onmessage = function (block) {
                    const object = JSON.parse(block.data);
                    callbacks(object);
                };


            } else {
                console.log("⚠️ Relay desconectado, ignorando envio.");
                socketsWithError.push(socket);
            }
        }

        // Remove os Relay desconectados
    ...
    connect: function (hosts) {
        this.sockets = [];
        this.isConnected = false; // Flag para indicar se pelo menos um Relay está ativo
        ...
        hosts.forEach((host) => {
            ...
            socket.onopen = () => {
                ...
                // Executa todas as funções pendentes que estavam esperando conexão
            ...
            socket.onerror = (error) => {
                console.error(`❌ Erro Relay (${host}):`, error);
            };

            socket.onclose = () => {
                console.log(`⚠️ Relay desconectado: ${host}`);
                this.sockets = this.sockets.filter(s => s !== socket);

                if (this.sockets.length === 0) {
                    this.isConnected = false; // Marca como desconectado se todos caírem
                }
            };
        });
    },
    ...
    sub: function (filters, callbacks) {
        ...
        // Se há conexão, envia a requisição
        if (this.isConnected && this.sockets.length > 0) {
            this.sockets.forEach((socket) => {
                socket.send(JSON.stringify({ request: "get_blocks", filter: filters, requestId }));

                socket.addEventListener("message", (block) => {
                    const object = JSON.parse(block.data);
                    if (object.requestId === requestId && object.response === "blocks" && Array.isArray(object.blocks)) {
                        callbacks({ response: "blocks", blocks: object.blocks });
                    }
                });
            });
        } else {
            // console.log("⏳ Aguardando Relay...");
...
// Criar bloco de **POST**
onpostt.createPost = function (content, privateKey, app) {
    const block = { mode: "post", content, query: [], app, ref: null };
    return this.signBlock(block, privateKey);
};

// Criar bloco de **REAÇÃO** (like, dislike, etc.)
onpostt.createReaction = function (emoji, ref, privateKey, app) {
    const block = { mode: "reaction", content: emoji, query: [], app, ref };
    return this.signBlock(block, privateKey);
};

// Criar bloco de **FOLLOW**
onpostt.createFollow = function (followedPubkey, privateKey, app) {
    const block = { mode: "follow", content: "", query: [], app, ref: followedPubkey };
    return this.signBlock(block, privateKey);
};

// Criar bloco de **MENSAGEM**
onpostt.createMessage = function (message, recipientPubkey, privateKey, app) {
    const block = { mode: "message", content: message, query: [], app, ref: recipientPubkey };
    return this.signBlock(block, privateKey);
};

// Criar bloco de **PROFLE**
onpostt.createProfile = function (profileData, privateKey, app) {
    const block = { mode: "profile", content: JSON.stringify(profileData), query: [], app, ref: null };
    return this.signBlock(block, privateKey);
};

// Criar bloco de **SEGUIDORES**
onpostt.createFollowing = function (privateKey, app) {
    const block = { mode: "following", content: "", query: [], app, ref: null };
    return this.signBlock(block, privateKey);
};