...
var onpostt = {
    callbacks: [],
    data: [],
    sockets: [], // Agora é um array para múltiplos Relay
    isConnected: false,
    callbacksQueue: [],
    ec: new elliptic.ec('secp256k1'), // Inicializa a curva elíptica secp256k1
    modes_list: ['post', 'follow', 'delete', 'like', 'comment', 'profile', 'payment'],
    modes_sub: ['message'],

    verificarHost: function(array, tipo) {
	    if (!Array.isArray(array) || typeof tipo !== 'string') {
	        return { host: false, url: '' };
	    }

	    const prefixo = tipo.toLowerCase() + '://';

	    const urlEncontrada = array.find(url => typeof url === 'string' && url.startsWith(prefixo));

	    if (urlEncontrada) {
	        return { host: true, url: urlEncontrada };
	    } else {
	        return { host: false, url: '' };
	    }
	},

	isValidModeList: function(mode) {
	  return this.modes_list.includes(mode);
	},

	isValidModeSub: function(mode) {
	  return this.modes_sub.includes(mode);
	},

	convertHostsWS: function (hosts, modo) {
	    if (!Array.isArray(hosts) || (modo !== 'http' && modo !== 'socket')) return [];

	    return hosts.map(url => {
    ...
    generateKeys: function () {
        ...
        return {
            ...
            publicKey: keyPair.getPublic(true, 'hex') // Compressão ativada
        };
    },

    generateKeysCustom: async function (input) {
    const hashHex = await this.sha256(input); // espera o hash resolver
    if (!hashHex || hashHex.length !== 64) {
        throw new Error("Hash inválido");
    }

	    // Transformar o hash em Uint8Array (buffer)
	    const hashArray = Uint8Array.from(
	        hashHex.match(/.{1,2}/g).map(byte => parseInt(byte, 16))
	    );

	    const keyPair = this.ec.keyFromPrivate(hashArray);
	    return {
	        privateKey: keyPair.getPrivate('hex'),
    ...
    // Gerar chave pública a partir de uma chave privada
    generatePublicKey: function (privateKey) {
        try {
            const keyPair = this.ec.keyFromPrivate(privateKey, 'hex');
            return keyPair.getPublic(true, 'hex'); // Compressão ativada
        } catch (e) {
            return "Chave privada inválida.";
        }
    },

    // Função para verificar a assinatura no frontend
    verifySignature: async function (publicKeyHex, message, signature) {
        // console.log('Verificando Assinatura:', publicKeyHex, message, signature);
        try {
            const key = this.ec.keyFromPublic(publicKeyHex, 'hex'); // Carrega a chave pública
            ...
            console.error('❌ Erro ao verificar a assinatura:', e);
            return false;
        }
    },
    ...
    signBlock: async function (block, privateKey) {
        if (typeof block !== 'object') return 'Bloco inválido';

        // Preenche informações do bloco
        block.created_at = Math.floor(Date.now() / 1000);
        block.pubkey = this.generatePublicKey(privateKey);
        ...
        if(block.mode === 'message'){
			...
			if (!fromEntry || !toEntry) {
		        return '❌ Campos from/to obrigatórios';
		    }
			
			  const fromValue = fromEntry[1];
			  const toValue = toEntry[1];

			  console.log("from:", fromValue);
			  console.log("to:", toValue);

			 contentEcrypted = await this.encryptMessage(toValue, fromValue, block.content, privateKey);
			
			 block.content = JSON.stringify(contentEcrypted);
        }

        delete block.sig; // Remove assinatura antiga (se existir)
        
        
        block.created_at = Math.floor(Date.now() / 1000);
        // Gera ID do bloco baseado no conteúdo (hash SHA-256)
        block.id = await this.sha256(JSON.stringify(block));
        
        
        // Converte o bloco para texto
        const blockText = JSON.stringify(block);
        const keyPair = this.ec.keyFromPrivate(privateKey, 'hex');

        // Gera o hash correto para assinatura
        const msgHash = await this.sha256(blockText);

        // Assina o hash da mensagem com a chave privada
        const signature = keyPair.sign(msgHash);

        // Converte a assinatura para o formato DER (hex)
        block.sig = signature.toDER('hex');

        // Valida a assinatura com a chave pública
        const isValid = await this.verifySignature(block.pubkey, blockText, block.sig);

        if (!isValid) {
            return '❌ Falha na validação da assinatura';
        }

        // console.log('✅ Bloco Assinado:', block);
        return block;
    },

	sendBlock: async function (block, callbacks) {
	    if (!this.isValidJsonObject(block)) {
	        callbacks({ status: 'error', message: 'Bloco inválido: não pode ser convertido para JSON.' });
	        return;
	    }

	    if (typeof block !== 'object') {
	        callbacks({ status: 'error', message: 'Bloco inválido!' });
	        return;
	    }
	    ...
	    const status = await this.verifySignature(block.pubkey, JSON.stringify(formatBlock), signature);
	    if (!status) {
	        callbacks({ status: 'error', message: 'Assinatura inválida, bloco rejeitado!' });
	        return;
	    }

	    // Envia via HTTP, se houver hosts disponíveis
	    if(this.isValidModeList(block.mode)){
	    	if (Array.isArray(this.hosts) && this.hosts.length > 0) {
		        for (const host of this.hosts) {
		            try {
		                const response = await fetch(host + '/save_block', {
		                    method: 'POST',
		                    headers: {
		                ...
		                callbacks({ status: 'error', message: err.message });
		            }
		        }
		    } 
	    }
	    ...
	    // Envia via WebSocket, se houver sockets abertos	
	    if(this.isValidModeSub(block.mode)){
	    	console.log('enviado');
		    if (Array.isArray(this.sockets) && this.sockets.length > 0) {
		        let socketsWithError = [];

		        for (let socket of this.sockets) {
		            if (socket.readyState === WebSocket.OPEN) {

		                socket.send(JSON.stringify(block));
		                console.log('✅ Bloco enviado via WebSocket');
		                ...
		                socket.onmessage = function (event) {
		                    try {
		                        ...
		                        callbacks({ status: 'error', message: 'Erro ao processar resposta do WebSocket' });
		                    }
		                };
		            } else {
		                console.log("⚠️ Relay WebSocket desconectado, ignorando envio.");
		                socketsWithError.push(socket);
		            }
		        }

		        // Remove sockets fechados
		        this.sockets = this.sockets.filter(socket => !socketsWithError.includes(socket));
    ...
    isValidJsonObject: function (obj) {
        try {
            // Tenta converter o objeto para JSON
            JSON.stringify(obj);
            return true; // Se não houver erro, o objeto é válido
        } catch (error) {
            console.error("❌ Bloco inválido:", error.message);
            return false; // Se ocorrer um erro, o objeto é inválido
        }
    },
    ...
    connect: function (servers) {
	    ...
	    this.callbacksQueue = this.callbacksQueue || [];
	    this.hosts = servers.host || [];


	    if(Array.isArray(servers.host)){
	    	servers.host.forEach((host) =>{
	    		console.log(`✅ Conectado ao host: ${host}`);
	    	})
	    }

	    if (Array.isArray(servers.socket)) {
	        servers.socket.forEach((url) => {
	            const socket = new WebSocket(url);

	            socket.onopen = () => {
	                console.log(`✅ Conectado ao socket: ${url}`);
	            ...
	            socket.onerror = (err) => {
	                console.error(`❌ Erro no socket (${url}):`, err);
	            };

	            socket.onclose = () => {
	                console.log(`⚠️ Socket desconectado: ${url}`);
    ...
    createConnection: function (host, onConnectedCallback = null) {
        const socket = new WebSocket(host);

        socket.onopen = () => {
            console.log(`Relay conectado: ${host}`);
            this.isConnected = true;
            this.sockets.push(socket); // Adiciona o Relay à lista
            while (this.callbacksQueue.length > 0) this.callbacksQueue.shift()();
            if (onConnectedCallback) {
                onConnectedCallback();
            }
        };

        socket.onerror = (error) => console.error(`Erro Relay (${host}):`, error);

        socket.onclose = () => {
            console.log(`Relay desconectado: ${host}`);
            this.isConnected = false;
	...
	encryptMessage: async function (receiverPubKeyHex, senderPubKeyHex, message, senderPrivKeyHex) {
	    try {
	        const ec = new elliptic.ec('secp256k1');

	        const senderKey = ec.keyFromPrivate(senderPrivKeyHex, 'hex');
	        const senderPubKey = senderKey.getPublic(true, 'hex');

	        const encryptFor = async (pubKeyHex) => {
	            const receiverKey = ec.keyFromPublic(pubKeyHex, 'hex');
	            const sharedSecret = senderKey.derive(receiverKey.getPublic()).toString(16);
	            const sharedSecretBuffer = new TextEncoder().encode(sharedSecret);

	            const hash = await crypto.subtle.digest('SHA-256', sharedSecretBuffer);
	        ...
	        return {
	            forRecipient: await encryptFor(receiverPubKeyHex),
	            forSender: await encryptFor(senderPubKeyHex)
	        };
	    } catch (e) {
	        console.error('Erro encryptMessageForBoth:', e.message);
	        throw e;
	    }
	},


	decryptMessage: async function (receiverPrivKeyHex, blocks) {
	    try {
	    	const contentMessage = blocks[0].content;
		    const encryptedContent = JSON.parse(contentMessage);
	        const ec = new elliptic.ec('secp256k1');
	        const receiverKey = ec.keyFromPrivate(receiverPrivKeyHex, 'hex');

	        // Função interna para tentar descriptografar uma entrada
	        const tryDecrypt = async (encrypted) => {
	            const senderPubKey = ec.keyFromPublic(encrypted.senderPubKey, 'hex');

	            const sharedSecret = receiverKey.derive(senderPubKey.getPublic()).toString(16);
	            const sharedSecretBuffer = new TextEncoder().encode(sharedSecret);
	            const hash = await crypto.subtle.digest('SHA-256', sharedSecretBuffer);
	        ...
	        // Tenta descriptografar com a parte do remetente
	        if (encryptedContent.forRecipient) {
	            try {
	                return await tryDecrypt(encryptedContent.forRecipient);
	            } catch (_) {}
	        }

	        // Se falhar, tenta com a parte do remetente (útil se for você mesmo lendo)
	        if (encryptedContent.forSender) {
	            try {
	                return await tryDecrypt(encryptedContent.forSender);
	            } catch (_) {}
	        }

	        throw new Error("Não foi possível descriptografar com esta chave privada.");

	    } catch (e) {
	    	// return 'Erro decryptMessage:'+ e.message
	        console.error('Erro decryptMessage:', e.message);
	        throw e;
	    }
	},

	decryptMessages: async function (receiverPrivKeyHex, blocks) {
	    const ec = new elliptic.ec('secp256k1');
	    const receiverKey = ec.keyFromPrivate(receiverPrivKeyHex, 'hex');
	    ...
	    // Converte blocks para array de conteúdos (JSON)
	    const mensagensCriptografadas = Object.values(blocks).map(b => {
	        try {
	            return typeof b.content === 'string' ? JSON.parse(b.content) : b.content;
	        } catch (err) {
	            console.error('Erro ao parsear conteúdo do bloco:', err.message);
	            return null;
	        }
	    ...
	    const tryDecrypt = async (encrypted) => {
	        try {
	            const senderPubKey = ec.keyFromPublic(encrypted.senderPubKey, 'hex');
	            const sharedSecret = receiverKey.derive(senderPubKey.getPublic()).toString(16);
	            const hash = await crypto.subtle.digest('SHA-256', encoder.encode(sharedSecret));
	            ...
	            console.error('Erro no tryDecrypt:', err.message);
	            return null;
	        }
	    };

	    const decryptPromises = mensagensCriptografadas.map(async (msg) => {
	        for (const key of ['forRecipient', 'forSender']) {
	            if (msg[key]) {
	                const result = await tryDecrypt(msg[key]);
	...
	sub: function (filters, callbacks) {
	    if (typeof filters === 'function') {
	        callbacks = filters;
	        filters = {};
	    }

	    if (filters.mode === 'message' && filters.privatekey) {
	        filters.pubkey = this.generatePublicKey(filters.privatekey);
	    }

	    // Armazena IDs já recebidos para evitar duplicatas
	    if (!this._receivedBlockIds) {
	        this._receivedBlockIds = new Set();
	    }

	    if (this.isConnected && Array.isArray(this.sockets) && this.sockets.length > 0) {
	        this.sockets.forEach((socket) => {
	            socket.send(JSON.stringify({ request: "get_blocks_sub", filter: filters }));

	            if (!socket._onMessageAdded) {
	                socket.addEventListener("message", async (event) => {
	                    try {
	                        const object = JSON.parse(event.data);

	                        if (object.response === "blocks" && Array.isArray(object.blocks)) {
	                            const newBlocks = [];

	                            for (let v of object.blocks) {
	                                if (!this._receivedBlockIds.has(v.id)) {
	                                    this._receivedBlockIds.add(v.id);
	                                    newBlocks.push(v);
	                                }
	                            }

	                            if (newBlocks.length > 0) {
	                                callbacks({ response: "blocks", blocks: newBlocks });
	                            }
	                        }
	                    } catch (err) {
	                ...
	                socket._onMessageAdded = true;
	            }
	        });
	    } else {
	        this.callbacksQueue.push(() => this.sub(filters, callbacks));
	    }
	},

	list: async function (filters, callbacks) {
	    ...
	    let newpub;
	    if (filters.mode === 'message') {
	        newpub = this.generatePublicKey(filters.privatekey);
	    } else {
	        newpub = filters.pubkey;
	    }
	    filters.pubkey = newpub;

	    const allBlocksMap = new Map(); // Mapa para evitar duplicados via v.id

	    for (const host of hostList) {
	        try {
	            const response = await fetch(host + '/get_blocks_list', {
	                method: 'POST',
	                headers: {
	                    'Content-Type': 'application/json',
	                },
	            ...
	            if (object.response === "blocks" && Array.isArray(object.blocks)) {
	                for (let v of object.blocks) {
	                    if (!allBlocksMap.has(v.id)) {
	                        allBlocksMap.set(v.id, v);
	                    }
	                }
	            }
	        } catch (err) {
	    ...
	    const unifiedBlocks = Array.from(allBlocksMap.values());
	    callbacks({ response: "blocks", blocks: unifiedBlocks });
	}
