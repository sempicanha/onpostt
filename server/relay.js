...
//         allowedTags: [ 'b', 'i', 'em', 'strong', 'a', 'p', 'ul', 'ol', 'li', 'br', 'span' ], // tags permitidas
//         allowedAttributes: {
//             '*': ['class', 'style'], // Atributos permitidos para todas as tags
//             'a': ['href', 'target'], // Atributos permitidos para a tag 'a'
//         },
//     });
// }
...
function saveBlock(block, ws) {
    try {
        ...
        // if (block.content) {
        //     block.content = sanitizeContent(block.content); // Sanitiza o conteúdo HTML
        // }
        ...
        if (block.mode === "delete") {
            ...
            if (block.mode === 'message') {
                ...
                if (isRecipient) {
                    ...
                    // logger.info('Salvando para o receptor:', recipientPubkey);
...
function getBlocks(filter, callback) {
    ...
    let results = files.map(file => {
        ...
        try {
            ...
            logger.error(`Erro ao processar ${file}: JSON inválido.`);
    ...
    ws.on("message", (message) => {
        try {
            ...
            // Se for um bloco válido, processa e salva
                ...
                logger.info("📢 Broadcast para todos os clientes:", data);
